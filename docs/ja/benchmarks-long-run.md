...
実稼働指向のストレス: 15 秒間のスモーク テストよりも長い
`benchmarks.md`、RSS サンプリングと不利なトラフィック パターン。

## 実行環境

- macOS 26.0.1、Apple Silicon (10コア)
- Zig 0.16.0 リリースファースト
- wrk 4.2.0、ループバック (`127.0.0.1`)
...
| `/hello` (16 キープアライブ) | 30代 | 161,112 | 40μs | 80μs | 16.7ミリ秒 | 0 / 4,849,334 | 1.9 → 2.4MB |
| `/hello` (1024 接続、`Connection: close`) | 60代 | 1,536 | 17.6ミリ秒 | 64.7ミリ秒 | 223ミリ秒 | 1024 作業側接続 |該当なし (下記を参照) |
