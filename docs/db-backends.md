...
d1_step: new WebAssembly.Suspending(async (h) => {
  const e = d1stmts.get(h);
  if (!e.iter) {
    const bound = e.base.bind(...e.bindArgs);
    const out = await bound.raw({ columnNames: true });
    e.columnNames = out[0];
    e.iter = out.slice(1)[Symbol.iterator]();
  }
  const next = e.iter.next();
  if (next.done) return 0;
  e.currentRow = next.value;
  return 1;
}),
// ...
...
JSPI の wasm stack switch 自体は **µs オーダー** で、D1 のネットワーク往復に対して無視できる。実測すべきは:
